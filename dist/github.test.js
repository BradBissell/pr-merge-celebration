...
vitest_1.vi.mock('@octokit/rest', () => {
    return {
        Octokit: vitest_1.vi.fn().mockImplementation(() => {
            return {
                pulls: {
                    list: vitest_1.vi.fn(),
                },
                paginate: {
                    iterator: vitest_1.vi.fn(),
                },
            };
        }),
    };
});
(0, vitest_1.describe)('GitHubClient', () => {
    let githubClient;
    let mockOctokit;
    (0, vitest_1.beforeEach)(() => {
        vitest_1.vi.clearAllMocks();
        githubClient = new github_1.GitHubClient('test-token');
        // Access the mocked Octokit instance
        mockOctokit = githubClient.octokit;
    });
    (0, vitest_1.describe)('getMergedPRsInTimeWindow', () => {
        (0, vitest_1.it)('should return empty array when no PRs are merged', async () => {
            const repos = [{ owner: 'octocat', repo: 'hello-world' }];
        ...
        (0, vitest_1.it)('should filter out PRs without merged_at timestamp', async () => {
            const repos = [{ owner: 'octocat', repo: 'hello-world' }];
            mockOctokit.paginate.iterator.mockImplementation(async function* () {
                yield {
                    data: [
        ...
        (0, vitest_1.it)('should return PRs merged in the last 24 hours', async () => {
            const repos = [{ owner: 'octocat', repo: 'hello-world' }];
            const now = new Date();
        ...
        (0, vitest_1.it)('should filter out PRs merged more than 24 hours ago', async () => {
            const repos = [{ owner: 'octocat', repo: 'hello-world' }];
            const now = new Date();
            const recentMerge = new Date(now.getTime() - 12 * 60 * 60 * 1000); // 12 hours ago
            ...
            const result = await githubClient.getMergedPRsInTimeWindow(repos);
            (0, vitest_1.expect)(result).toHaveLength(1);
            (0, vitest_1.expect)(result[0].number).toBe(123);
            (0, vitest_1.expect)(result[0].title).toBe('Recent PR');
        });
        (0, vitest_1.it)('should handle multiple repositories', async () => {
            const repos = [
            ...
            (0, vitest_1.expect)(result[0].repository).toBe('microsoft/vscode'); // More recent, appears first
            (0, vitest_1.expect)(result[1].repository).toBe('octocat/hello-world');
        });
        (0, vitest_1.it)('should sort PRs by merged date (most recent first)', async () => {
            const repos = [{ owner: 'octocat', repo: 'hello-world' }];
            const now = new Date();
            ...
            (0, vitest_1.expect)(result[0].number).toBe(123); // Newest first
            (0, vitest_1.expect)(result[1].number).toBe(122);
            (0, vitest_1.expect)(result[2].number).toBe(121); // Oldest last
        });
        (0, vitest_1.it)('should handle missing user information gracefully', async () => {
            const repos = [{ owner: 'octocat', repo: 'hello-world' }];
            const now = new Date();
            ...
            const result = await githubClient.getMergedPRsInTimeWindow(repos);
            (0, vitest_1.expect)(result).toHaveLength(1);
            (0, vitest_1.expect)(result[0].author).toBe('Unknown');
            (0, vitest_1.expect)(result[0].authorAvatar).toBe('');
        });
        (0, vitest_1.it)('should handle API errors gracefully and continue with other repos', async () => {
            const repos = [
                { owner: 'octocat', repo: 'hello-world' },
                { owner: 'microsoft', repo: 'vscode' },
            ];
            ...
            const result = await githubClient.getMergedPRsInTimeWindow(repos);
            (0, vitest_1.expect)(result).toHaveLength(1);
            (0, vitest_1.expect)(result[0].repository).toBe('microsoft/vscode');
            (0, vitest_1.expect)(consoleErrorSpy).toHaveBeenCalledWith(vitest_1.expect.stringContaining('Error fetching PRs for octocat/hello-world:'), vitest_1.expect.any(Error));
            consoleErrorSpy.mockRestore();
        });
        (0, vitest_1.it)('should respect custom merge window parameter', async () => {
            const repos = [{ owner: 'octocat', repo: 'hello-world' }];
            const now = new Date();
            ...
            (0, vitest_1.expect)(result).toHaveLength(1);
            (0, vitest_1.expect)(result[0].number).toBe(123);
            (0, vitest_1.expect)(result[0].title).toBe('Recent PR (6 hours ago)');
        });
        (0, vitest_1.it)('should default to 24 hours when no window is specified', async () => {
            const repos = [{ owner: 'octocat', repo: 'hello-world' }];
            const now = new Date();
    ...
    (0, vitest_1.describe)('pagination behavior', () => {
        (0, vitest_1.it)('should stop fetching after reaching safety limit of 1000 PRs', async () => {
            const repos = [{ owner: 'octocat', repo: 'hello-world' }];
            const now = new Date();
            const recentMerge = new Date(now.getTime() - 12 * 60 * 60 * 1000);
            ...
            (0, vitest_1.expect)(result.length).toBeLessThanOrEqual(1000);
            // Verify the safety limit message was logged
            (0, vitest_1.expect)(consoleLogSpy).toHaveBeenCalledWith(vitest_1.expect.stringContaining('Reached safety limit of 1000 PRs'));
            consoleLogSpy.mockRestore();
        });
        (0, vitest_1.it)('should stop fetching when PRs become older than cutoff time', async () => {
            const repos = [{ owner: 'octocat', repo: 'hello-world' }];
            const now = new Date();
            ...
            (0, vitest_1.expect)(result).toHaveLength(1);
            (0, vitest_1.expect)(result[0].number).toBe(1);
            ...
            (0, vitest_1.expect)(pageCount).toBeLessThanOrEqual(2);
        });
        (0, vitest_1.it)('should handle multiple pages of recent PRs', async () => {
            const repos = [{ owner: 'octocat', repo: 'hello-world' }];
            const now = new Date();
            ...
            (0, vitest_1.expect)(result.map((pr) => pr.number)).toEqual([1, 2, 3, 4]);
        });
        (0, vitest_1.it)('should handle empty pages gracefully', async () => {
            const repos = [{ owner: 'octocat', repo: 'hello-world' }];
            mockOctokit.paginate.iterator.mockImplementation(async function* () {
                yield { data: [] };
            });
        ...
        (0, vitest_1.it)('should continue pagination when page has PRs within cutoff', async () => {
            const repos = [{ owner: 'octocat', repo: 'hello-world' }];
            const now = new Date();
